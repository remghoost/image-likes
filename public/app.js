...
function imageCardHtml(post, forModal = false) {
  ...
  const media = multi
    ? carouselHtml(images)
    : `<div class="image-wrap"><img src="/uploads/${images[0].filename}" alt="Shared image" loading="lazy"></div>`;
  ...
  const slides = images.map((img, i) => `
    <div class="carousel-slide">
      <div class="image-wrap">
        <img src="/uploads/${img.filename}" alt="Shared image ${i + 1} of ${images.length}" loading="lazy">
      </div>
    </div>`).join('');
...
function attachCardEvents(post, root) {
  ...
  // Double tap / double click to like (or unlike) — works on every image in the post.
  const attachDoubleTap = (imageEl) => {
    let lastTap = 0;
    const handleDoubleTap = (e) => toggleLike(post, card, e);
    imageEl.addEventListener('dblclick', handleDoubleTap);
  ...
  card.querySelectorAll('.image-wrap img').forEach(attachDoubleTap);
...
function gridTileHtml(post) {
  ...
  return `
    <div class="grid-tile" data-id="${post.id}" title="${escapeHtml(post.description || 'View post')}">
      <img src="/uploads/${images[0].filename}" alt="Post" loading="lazy">
...
function updateModalOpenState() {
  const anyOpen = ['pw-modal', 'edit-profile-modal', 'crop-modal', 'post-modal', 'edit-desc-modal']
    .some((id) => !document.getElementById(id).classList.contains('hidden'));
  document.body.classList.toggle('modal-open', anyOpen);
}

// Close whichever modal is currently open (Escape key).
